...
public class ConsultaDAOimpl extends ConexionDB implements ConsultaDAO {
    ...
    public List<Consulta> getAll() throws Exception {
        ...
        try {
            ...
            sql += "FROM CONSULTAS C LEFT JOIN DOCTORES D ON C.id_doctor = D.id LEFT JOIN PACIENTES P ON C.id_paciente = P.id";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                con.setSintomas(rs.getString("sintomas"));
                con.setDiagnostico(rs.getString("diagnostico"));
                con.setDoctor(rs.getString("doctor"));
                con.setPaciente(rs.getString("paciente"));
                con.setSex_paciente(rs.getString("sex_paciente"));
