...
public class RecetaDAOimpl extends ConexionDB implements RecetaDAO {
    ...
    public Receta getById(int id) throws Exception {
        ...
        try {
            ...
            sql += "LEFT JOIN PACIENTES P ON C.id_paciente=P.id WHERE R.id=?";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ps.setInt(1, id);
            ...
            if (rs.next()) {
                ...
                rec.setDoctor(rs.getString("doctor"));
                rec.setPaciente(rs.getString("paciente"));
            }
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public List<Receta> getAll() throws Exception {
        ...
        try {
            ...
            sql += "FROM RECETAS R LEFT JOIN CONSULTAS C ON R.id_consulta=C.id LEFT JOIN DOCTORES D ON C.id_doctor=D.id ";
            sql += "LEFT JOIN PACIENTES P ON C.id_paciente=P.id";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
