...
public class ReservaDAOimpl extends ConexionDB implements ReservaDAO{
    ...
    public List<Reserva> getAll() throws Exception {
        ...
        try {
            ...
            sql += "LEFT JOIN PACIENTES P ON R.id_paciente=P.id LEFT JOIN DOCTORES D ON R.id_doctor=D.id";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                res.setHora(rs.getTime("hora"));
                res.setObservacion(rs.getString("observacion"));
                res.setPaciente(rs.getString("paciente"));
                res.setDoctor(rs.getString("doctor"));
