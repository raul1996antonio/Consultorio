...
public class DoctorDAOimpl extends ConexionDB implements DoctorDAO {
    ...
    public void insert(Doctor doctor) throws Exception {
        String sql = "INSERT INTO DOCTORES (id, ci, nombre, ap_paterno, ap_materno, fecha_nacimiento, sexo, telefono, direccion, ciudad, correo, id_turno, id_estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            this.Conectar();
            ...
            ps.setInt(13, doctor.getId_estado());
            ps.executeUpdate();
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public void update(Doctor doctor) throws Exception {
        String sql = "UPDATE DOCTORES SET id=?, ci=?, nombre=?, ap_paterno=?, ap_materno=?, fecha_nacimiento=?, sexo=?, telefono=?, direccion=?, ciudad=?, correo=?, id_turno=?, id_estado=? WHERE id=?";
        try {
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ps.setInt(1, doctor.getId());
    ...
    public Doctor getById(int id) throws Exception {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                doc.setId_estado(rs.getInt("id_estado"));
            }
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public List<Doctor> getAll() throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                doc.setId_turno(rs.getInt("id_turno"));
                doc.setId_estado(rs.getInt("id_estado"));
                doc.setTurno(rs.getString("turno"));
