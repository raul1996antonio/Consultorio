...
public class RecepcionistaDAOimpl extends ConexionDB implements RecepcionistaDAO{
    ...
    public void insert(Recepcionista recepcionista) throws Exception {
        String sql = "INSERT INTO RECEPCIONISTAS (id, ci, nombre, ap_paterno, ap_materno, fecha_nacimiento, sexo, telefono, direccion, ciudad, correo, id_turno, id_estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            this.Conectar();
            ...
            ps.setInt(13, recepcionista.getId_estado());
            ps.executeUpdate();
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public void update(Recepcionista recepcionista) throws Exception {
        String sql = "UPDATE RECEPCIONISTAS SET id=?, ci=?, nombre=?, ap_paterno=?, ap_materno=?, fecha_nacimiento=?, sexo=?, telefono=?, direccion=?, ciudad=?, correo=?, id_turno=?, id_estado=? WHERE id=?";
        try {
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ps.setInt(1, recepcionista.getId());
            ...
            ps.setInt(12, recepcionista.getId_turno());
            ps.setInt(13, recepcionista.getId_estado());
            ps.setInt(14, recepcionista.getId());
            ps.executeUpdate();
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public List<Recepcionista> getAll() throws Exception {
        ...
        try {
            ...
            sql += "LEFT JOIN ESTADOS E ON R.id_estado = E.id";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                res.setId_turno(rs.getInt("id_turno"));
                res.setId_estado(rs.getInt("id_estado"));
                res.setTurno(rs.getString("turno"));
